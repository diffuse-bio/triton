...
class DropoutResAddCLN(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, z, x, normalized_shape, p=0.5, seed=42, eps=1e-5):
        ...
        _drcln_fwd_fused[(M,)](x_arg, y, z, z_out, p, seed, #mean, rstd,
...
def test_drcln(M, N, dtype, eps=1e-5, device='cuda'):
    ...
    z_out_tri = dracln(x, x, w_shape) #, p=0.5, seed=123)
    print(z_out_tri[0], z_out_tri[1], z_out_tri[-1])
    # y_tri = layer_norm(x, w_shape, weight, bias, eps)
    # y_ref = vanilla_conditional_drcln(x, weight, bias, eps).to(dtype) #torch.nn.functional.layer_norm(x, w_shape, weight, bias, eps).to(dtype)
